...
main( int     argc,
  ...
  int flag_pmc        = fd_env_strip_cmdline_contains( &argc, &argv, "--pmc"             );
  char const * img    = fd_env_strip_cmdline_cstr    ( &argc, &argv, "--img", NULL, NULL );

  if( FD_UNLIKELY( !img ) ) FD_LOG_ERR(( "missing --img argument" ));
  struct stat st;
  if( FD_UNLIKELY( 0!=stat( img, &st ) ) ) {
    FD_LOG_ERR(( "stat(%s) failed (%i-%s)", img, errno, fd_io_strerror( errno ) ));
  }
  ulong fdos_kern_img_sz = (ulong)st.st_size;
  uchar * fdos_kern_img = aligned_alloc( FD_SHMEM_NORMAL_PAGE_SZ, fdos_kern_img_sz );
  FD_TEST( fdos_kern_img );
  FILE * f = fopen( img, "rb" ); FD_TEST( f );
  size_t n = fread( fdos_kern_img, 1, st.st_size, f ); FD_TEST( n==st.st_size );
  fclose( f );
  ...
  if( flag_pmc ) {
    if( FD_UNLIKELY( ioctl( vm_fd, KVM_CREATE_IRQCHIP, 0 )<0 ) ) {
      FD_LOG_ERR(( "KVM_CREATE_IRQCHIP failed (%i-%s)", errno, fd_io_strerror( errno ) ));
    }
  }
